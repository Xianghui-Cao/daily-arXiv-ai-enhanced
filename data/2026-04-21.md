...
TL;DR: The paper explores the use of the deterministic quantum imaginary time evolution (QITE) algorithm to find the ground state of a 2D pure $\mathbb{Z}_2$ lattice gauge theory, showing it can achieve high accuracy with reduced costs.


<details>
  <summary>Details</summary>
Motivation: The motivation is to develop a more efficient and accurate method for computing the ground state of a 2D pure $\mathbb{Z}_2$ lattice gauge theory that adheres to Gauss's law, while reducing measurement and gate costs in the QITE process.

Method: The authors construct a set of Pauli operators that commute with Gauss's law, apply the deterministic QITE algorithm, and perform classical numerical simulations using tensor networks. The results are compared with DMRG for validation.

Result: The deterministic QITE algorithm achieves a relative error of less than 0.1% for systems up to twelve plaquettes and within the studied coupling regime. The study also examines the impact of the number of time steps on the error.

Conclusion: The deterministic QITE algorithm is an effective and efficient approach for calculating the ground state of a 2D pure $\mathbb{Z}_2$ lattice gauge theory, with high precision and reduced computational costs.
